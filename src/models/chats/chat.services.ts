...
export class ChatService {
  ...
  async getNameFromChat(id: String, type: String): Promise<string | null> {
    try {
      if (type === 'user')
        return await User.findById(id).then((user) => user?.name || null);
      if (type === 'worker')
        return await Worker.findById(id).then((worker) => worker?.name || null);
      if (type === 'location')
        return await Location.findById(id).then(
          (location) => location?.nombre || null
        );
      if (type === 'business')
        return await Business.findById(id).then(
  ...
  async checkIfSenderInChat(
    chatId: string,
    userFrom: string
  ): Promise<Boolean> {
    const chat: IChat | null = await Chat.findById(chatId);
    if (chat === null) throw new Error('Chat not found');
    const name1 = await this.getNameFromChat(
      chat.user1.toString(),
      chat.typeOfUser1
    );
    if (name1 && name1 === userFrom) return true;
    const name2 = await this.getNameFromChat(
      chat.user2.toString(),
      chat.typeOfUser2
    );
  ...
  async sendMessage(
    ...
    const chat = await Chat.findByIdAndUpdate(
      chatId,
      { $push: { messages: messageToBeSaved } },
      { new: true }
    );
    if (chat) return true;
    else return false;
  ...
  async getPeopleWithWhomUserChatted(
    userId: string
  ): Promise<[string, string, typeOfXatUser, string][]> {
    const user = await User.findById(userId);
    ...
    const peopleInfo = await Promise.all(
      chats.map(async (chat) => {
        ...
        if (!name) return null;
        return [name, otherId, otherType, selfId] as [
          string,
          string,
          typeOfXatUser,
          string,
    ...
    return peopleInfo.filter(
      (entry): entry is [string, string, typeOfXatUser, string] =>
        entry !== null
    );
  }
  ...
  async getNameFromOtherPersonInChat(
    chatId: string,
    name: string
  ): Promise<string | null> {
  ...
  async getPeopleWithWhomWorkerChatted(
    workerId: string
  ): Promise<[string, string, typeOfXatUser, string][]> {
    const worker = await Worker.findById(workerId);
    ...
    const chats = await Chat.find({
      $or: [
        { user1: workerId },
        { user2: workerId },
        { user2: worker.location },
        { user1: worker.location },
        { user2: location.business },
        { user1: location.business },
      ],
    });
    /*         const people = chats.map(chat => chat.user1.toString() === userId ? chat.user2.toString() : chat.user1.toString());
            const peopleNames = await Promise.all(people.map(async (personId) => { 
    ...
    const peopleInfo = await Promise.all(
      chats.map(async (chat) => {
        ...
        if (!name) return null;
        return [name, otherId, otherTypeForFrontend, selfId] as [
          string,
          string,
          typeOfXatUser,
          string,
