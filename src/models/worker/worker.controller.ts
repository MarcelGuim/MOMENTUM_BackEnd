...
export async function createWorker(
  ...
  try {
    ...
    if (!adminId)
      return res.status(405).json({ message: 'Invalid worker ID format' });
    const worker: Partial<IWorker> = req.body.worker as IWorker;
    worker.isDeleted = false;
    const locationName = req.body.location;
    if (!locationName)
      return res.status(405).json({ error: 'Location not sent' });
    await workerService.createWorkerByAdmin(
      worker,
      adminId,
...
export async function createWorkerWithMultipleLocations(
  req: Request,
  res: Response
): Promise<Response> {
  try {
    ...
    const newWorker = await workerService.createWorkerWithMultipleLocations(
      worker,
      adminId
    );
    ...
    console.error(
      'Error creating worker with multiple locations:',
      error.message
    );
    if (typeof error === 'object' && error !== null && 'message' in error) {
      console.log((error as { message: string }).message);
...
export async function diguesHola(
  req: Request,
  res: Response
): Promise<Response> {
  console.log('Hola');
...
export async function hardDeleteWorkerById(
  req: Request,
  res: Response
): Promise<Response> {
  try {
...
export async function softDeleteWorkerById(
  req: Request,
  res: Response
): Promise<Response> {
  try {
...
export async function softDeleteWorkersByIds(
  req: Request,
  res: Response
): Promise<Response> {
  try {
    ...
    if (result === workersNum) {
      ...
      return res
        .status(404)
        .json({ error: `Only ${result} workers soft deleted successfully` });
    }
  } catch {
    return res.status(500).json({ error: 'Failed to soft delete worker' });
  }
...
export async function restoreWorkerById(
  req: Request,
  res: Response
): Promise<Response> {
  try {
...
export async function getWorkersPaginated(
  req: Request<{}, {}, {}, PaginatedWorkersQueryParams>,
  res: Response
): Promise<Response> {
  try {
    ...
    const result = await workerService.getWorkersPaginated(
      page,
      limit,
      getDeleted
    );
...
export async function getWorkersByCompany(
  req: Request,
  res: Response
): Promise<Response> {
  try {
...
export async function getWorkersPaginatedByCompany(
  req: Request,
  res: Response
): Promise<Response> {
  try {
    ...
    const getDeleted = req.query.getDeleted == 'true';
    const { company } = req.params;
    const result = await workerService.getWorkersPaginatedByCompany(
      page,
      limit,
      getDeleted,
      company
...
export async function getWorkersByBusinessId(
  req: Request,
  res: Response
): Promise<Response> {
  try {
