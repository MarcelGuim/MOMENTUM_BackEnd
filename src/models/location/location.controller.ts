...
export async function createLocationHandler(
  ...
  try {
    ...
    if (location === 0) {
      return res
        .status(400)
        .json({ message: 'Error with data format in schedule' });
    }
    if (location === 1) {
      console.error('ServiceTypes of location are not valid');
      return res
        .status(400)
        .json({ message: 'ServiceTypes of location are not valid' });
    } else {
...
export async function softDeleteLocaitonHandler(
  req: Request,
  res: Response
): Promise<Response> {
  const { id } = req.params;
  ...
  try {
    const ok = await locationService.softDeleteLocation(id);
    if (ok) return res.status(200).json({ message: 'success' });
...
export async function restoreLocationHandler(
  req: Request,
  res: Response
): Promise<Response> {
  const { id } = req.params;
  ...
  try {
    const ok = await locationService.restoreLocation(id);
    if (ok) return res.status(200).json({ message: 'success' });
    return res.status(404).json({ error: 'could not find location' });
  } catch {
    return res.status(500).json({ error: 'internal error' });
...
export async function getAllLocationsByServiceTypeHandler(
  req: Request,
  res: Response
): Promise<Response> {
  ...
  try {
    if (
      !Object.values(locationServiceType).includes(
        serviceType as locationServiceType
      )
    ) {
      return res.status(400).json({ error: 'Invalid service type' });
    }
    const locations = await locationService.getAllLocationsByServiceType(
      serviceType as locationServiceType
    );
    return res.json(locations);
  } catch {
...
export async function getLocationsNearHandler(
  req: Request,
  res: Response
): Promise<Response> {
  ...
  if (!lat || !lon || !distance || !serviceType) {
    return res.status(400).json({
      error: 'Missing query parameters: lat, lon, distance, or serviceType',
    });
  }
  ...
  if (
    ...
    isNaN(parseFloat(distance as string))
  ) {
    return res
      .status(400)
      .json({ error: 'lat, lon, and distance must be valid numbers' });
  }
  ...
  if (
    ...
    return res.status(400).json({
      error:
        ...
        Object.values(locationServiceType).join(', '),
    });
  }
  ...
  try {
    ...
    if (results.length === 0) {
      return res
        .status(200)
        .json({ message: 'No nearby locations found', results });
    }
...
export async function getPlacesHandler(
  ...
  try {
    ...
    const places = await locationService.getPlaces(
      lonmin_f,
      latmin_f,
      lonmax_f,
      latmax_f,
...
export async function getRouteHandler(
  ...
  try {
    ...
    const route = await locationService.getRouteInfo(
      lon1_f,
      lat1_f,
      lon2_f,
      lat2_f,
...
export async function getWorkersOfLocation(
  req: Request,
  res: Response
): Promise<Response> {
  try {
...
export async function getLocationByNameHandler(
  req: Request,
  res: Response
): Promise<Response> {
  try {
...
export async function getBussinessIdFromLocationId(
  req: Request,
  res: Response
): Promise<Response> {
  try {
  ...
  try {
    ...
    if (cities === -1) {
      ...
      return res.status(500).json({
        message:
          'Internal server error while extracting cities. Check address formats.',
      });
    }
