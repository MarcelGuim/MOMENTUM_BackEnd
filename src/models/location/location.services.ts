...
export class LocationService {
    ...
    async updateLocationById(id: string, data: Partial<ILocation>): Promise<ILocation | null> {
        return await Location.findByIdAndUpdate(id, data, { new: true });
    }

    async getAllLocationsByServiceType(serviceType: locationServiceType): Promise<ILocation[]> {
        const locations = await Location.find({ serviceType: serviceType });
        return locations;
    }

    async getLocationsNearByServiceType(lat: number, lon: number, maxDistanceKm: number,serviceType: locationServiceType): Promise<ILocation[]> {
    ...
    async getPlaces(lonmin: number, latmin: number, lonmax: number, latmax: number, query: string): Promise<PlaceQueryResult[]>{
    ...
    async getRouteInfo(lon1: number, lat1: number, lon2: number, lat2: number, travelMode: TravelMode) {
    ...
    async getBusinessIdFromLocationId(locationId: string): Promise<string | null> {
