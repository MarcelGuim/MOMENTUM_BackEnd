...
export class CalendarService {
  ...
  async getAppointmentsForADay(
    ...
    return await this.getAppointmentsBetweenDates(
      startOfDay,
      endOfDay,
      calendarId
    );
  ...
  async addAppointmentToCalendar(
    calendarId: string,
  ...
  async softDeleteCalendarUser(calendarId: string): Promise<ICalendar | null> {
    ...
    const res = await Promise.all([
      Calendar.findByIdAndUpdate(calendarId, { $set: { isDeleted: true } }),
  ...
  async editCalendar(
    calendarId: string,
    changes: Partial<ICalendar>
  ): Promise<ICalendar | null> {
    const result = await Calendar.findByIdAndUpdate(calendarId, changes, {
      new: true,
    });
    return result;
  }

  async getEmptySlotForAUser(
    userId: string,
    date1: Date,
    date2: Date
    ...
    if (user?._id) {
      ...
      populatedCalendars.forEach((populatedCalendar) => {
        const calendarAppointments =
          populatedCalendar.appointments as unknown as IAppointment[];
      ...
      appointmentsInRange.sort(
        (a, b) => a.inTime.getTime() - b.inTime.getTime()
      );
      let slots: [Date, Date][] = [];
      ...
      appointmentsInRange.forEach((appointmentsInRange1, i) => {
        if (i < appointmentsInRange.length - 1) {
          slots.push([
            appointmentsInRange1.outTime,
            appointmentsInRange[i + 1].inTime,
          ]);
        }
      });
      slots.push([
        appointmentsInRange[appointmentsInRange.length - 1].outTime,
        endOfPeriod,
      ]);
      return slots;
    } else return null;
  ...
  async getMatchingDatesForTwoEmptySlotsArrays(
    array1: [Date, Date][],
    array2: [Date, Date][]
  ): Promise<[Date, Date][]> {
    ...
    for (const item1 of array1) {
      for (const item2 of array2) {
        const start = new Date(
          Math.max(item1[0].getTime(), item2[0].getTime())
        );
  ...
  async getMachingDatesForNEmptySoltsArrays(
    arrays: [Date, Date][][]
  ): Promise<[Date, Date][] | null> {
    let result: [Date, Date][] | null = arrays[0];
    for (let array of arrays) {
      result = await this.getMatchingDatesForTwoEmptySlotsArrays(
        result as [Date, Date][],
        array
      );
    }
  ...
  async getSlotsCommonForTwoUserCalendars(
    user1Id: string,
    user2Id: string,
    date1: Date,
    date2: Date
  ): Promise<[Date, Date][] | null | number> {
    ...
    if (user1._id && user2._id) {
      emptySlotsUser1 = await this.getEmptySlotForAUser(
        user1._id.toString(),
        date1,
        date2
      );
      emptySlotsUser2 = await this.getEmptySlotForAUser(
        user2._id.toString(),
        date1,
        date2
      );
  ...
  async getSlotsCommonForCalendarsOfNUsers(
    userIDs: string[],
    date1: Date,
    date2: Date
  ): Promise<[Date, Date][] | null | number | [number, string[]]> {
  ...
  async logDatesInISOFormat(
    dates: [Date, Date][],
    message: string
  ): Promise<void> {
    let datesInISO: [string, string][] = dates.map((date) => [
      date[0].toISOString(),
      date[1].toISOString(),
    ]);
  ...
  async setAppointmentRequestForWorker(
    ...
    await Promise.all(
      calendarsWorker.map(async (calendar) => {
        let result = await this.getAppointmentsBetweenDates(
          appointment.inTime,
          appointment.outTime,
    ...
    let result = await this.getAppointmentsBetweenDates(
      appointment.inTime,
      appointment.outTime,
      calendarUser?.id.toString()
    );
    if (result.length !== 0) throw new Error('Slot already taken');
    if (appointment.appointmentState !== appointmentState.REQUESTED)
      appointment.appointmentState = appointmentState.REQUESTED;
    const appointmentSaved = await new Appointment(appointment).save();
    this.addAppointmentToCalendar(userCalendarId, appointmentSaved);
    this.addAppointmentToCalendar(
      calendarsWorker[0]._id.toString(),
      appointmentSaved
    );
    return true;
  }

  async getSlotsCommonForCalendarsOfOneUserAndOneWorker(
    userId: string,
    workerId: string,
    date1: Date,
    date2: Date
    ...
    if (user._id && worker._id) {
      emptySlotsUser = await this.getEmptySlotForAUser(
        user._id.toString(),
        date1,
        date2
      );
      emptySlotsWorker = await this.getEmptySlotForAWorker(
        worker._id.toString(),
        date1,
        date2
      );
      ...
      else if (emptySlotsUser.length === 0)
  ...
  async getSlotsCommonForCalendarsOfOneUserAndOneLocation(
    userId: string,
    locationId: string,
    date1: Date,
    date2: Date
  ): Promise<[String, [Date, Date][]][]> {
    const user: IUsuari | null = await User.findById(userId);
    const location: ILocation | null = await Location.findById(locationId);
    if (!user || !location)
      throw new Error('Error finding the user or the bussiness');
    const workers: IWorker[] | null =
      await this.locationService.getWorkersOfLocation(locationId);
    if (!workers) throw new Error('Error finding the workers of the location');
    if (workers.length === 0)
      throw new Error('Error, there are no workers at that location');
    ...
    if (user._id && location._id) {
      for (const worker of workers) {
        if (worker._id) {
          try {
            const result =
              await this.getSlotsCommonForCalendarsOfOneUserAndOneWorker(
                userId,
                worker._id.toString(),
                date1,
                date2
  ...
  async getSlotsCommonForCalendarsOfOneUserAndOneBussiness(
    serviceType: string,
    userId: string,
    bussinessId: string,
    date1: Date,
    ...
    if (!user || !bussiness)
      throw new Error('Error finding the user or the bussiness');
    const locations: ILocation[] | null | number =
      await this.bussinessService.getAllLocationsFromBusinessbyServiceType(
        bussinessId,
        serviceType
      );
    if (!locations) throw new Error('Error finding the locations');
      ...
      if (locations === -1) throw new Error('Error in the bussiness Id');
        ...
        throw new Error(
          'There are no locations for this service type in the bussiness'
        );
    } else {
      if (user._id && bussiness._id) {
        for (const location of locations) {
          if (location._id) {
            try {
              const result =
                await this.getSlotsCommonForCalendarsOfOneUserAndOneLocation(
                  userId,
  ...
  async planAppointmentUsingAI(
    userId: string,
    prompt: string,
    max: number
  ): Promise<AppointmentPlanningResponse> {
  ...
  async acceptRequestedAppointment(
    appointmentId: string
  ): Promise<boolean | IAppointment> {
    const appointment: IAppointment | null =
      await Appointment.findById(appointmentId);
    if (!appointment) throw new Error('Appointment not found');
    if (
      appointment.appointmentState != appointmentState.STANDBY &&
      appointment.appointmentState != appointmentState.REQUESTED
    )
      throw new Error("Appointment can't be accepted, it already is");
    const answer: IAppointment | null = await Appointment.findByIdAndUpdate(
      appointmentId,
  ...
  async acceptStandByAppointment(
    appointment: IAppointment,
    userId: string
  ): Promise<boolean | IAppointment> {
    const user: IUsuari | null = await User.findById(userId);
    if (!user) throw new Error('User not found');
    const calendar: ICalendar | null = await Calendar.findOne({
      owner: user._id,
    });
