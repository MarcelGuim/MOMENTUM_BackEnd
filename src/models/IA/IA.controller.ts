...
export async function TestConnection(
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: 'Unexpected error testing connection' });
  }
}

export async function optimizeAppointments(
  req: Request,
  res: Response
): Promise<Response> {
  ...
  console.log(
    'Getting all appointments of user by date from IA for user ID:',
    userId
  );
  try {
    const formatedRequests: [Date, string[]][] =
      await IaService.getDayAndrequestsFromUserPetition(textToOptimize, userId);
    const answer: IAppointment[] = await IaService.getOptimizedSchedule(
      formatedRequests,
      userId
    );
    ...
    return res
      .status(500)
      .json({ error: 'Unexpected error getting appointments' });
  }
}
