...
export class BusinessService {
  ...
  async getAllBusiness(): Promise<IBusiness[] | null> {
    const businesses = await Business.find({ isDeleted: false }).populate(
      'location'
    );
    return businesses;
  }
  ...
  async getAllLocationsFromBusinessbyServiceType(
    businessId: string,
    serviceType: string
  ): Promise<ILocation[] | null | number> {
    // Mirem si el id té un format vàlid
  ...
  async getAllBusinesswithLocationofferingServiceType(
    serviceType: string
  ): Promise<IBusiness[] | null | number> {
  ...
  async createLocationForBusiness(
    workerId: string,
    ...
    if (!worker.businessAdministrated)
      throw new Error('Wrong business id for the administrator');
  ...
  async deleteLocationForBusiness(
    businessId: string,
    locationId: string
  ): Promise<IBusiness | null | number> {
  ...
  async hardDeleteBusiness(
    ...
    const business = await Business.findOne({ _id: businessId }).setOptions({
      bypassHooks: true,
    });
    if (!business) {
      return null;
  ...
  async findBusinessOrByLocationName(
    name: string
